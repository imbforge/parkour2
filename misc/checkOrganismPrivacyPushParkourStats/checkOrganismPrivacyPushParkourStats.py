...
# Configs
CONFIG = {
    "PARKOUR_BASE_URL": os.environ.get("PARKOUR_BASE_URL", "https://parkour.url.com/"),
    "PARKOUR_USER": os.environ.get("PARKOUR_USER", "parkourUserEmail"),
    "PARKOUR_USER_PASSWORD": os.environ.get(
        "PARKOUR_USER_PASSWORD", "parkourUserPassword"
    ),
    ...
    "PARKOUR_FROM_ADDRESS": os.environ.get(
        "PARKOUR_FROM_ADDRESS", "sender@example.com"
    ),
    "PARKOUR_TO_ADDRESSES": os.environ.get(
        "PARKOUR_TO_ADDRESSES", "alias1@example.com,alias2@example.com"
    ).split(","),
    "PRIVACY_PROTECTED_ORGANISMS": os.environ.get(
        "PRIVACY_PROTECTED_ORGANISMS", "Human donor,Secret bacterium"
    ).split(","),
}
...
def parse_cmd_args():
    ...
    description = """
                     (1) Check if any sample comes from an organism for which
                     the resulsting data is privacy sensitive. If so, flag
                     project with file _TO_BE_PRIVACY_PROTECTED
                     (2) Push run and demultiplexing stats to Parkour.
                     The following environment variables must be set:
                       ...
                       PRIVACY_PROTECTED_ORGANISMS, e.g. Human donor,Secret bacterium
                  """

    cmd_arg_parser = argparse.ArgumentParser(
        prog="pushParkourStats", description=description
    )
...
def sendMail(message, config):
    ...
    mailer.attach(email)
    s = smtplib.SMTP(
        config.get("PARKOUR_EMAIL_HOST"), port=config.get("PARKOUR_EMAIL_PORT", 25)
    )
    s.sendmail(
...
def pushParkour(
    flowcellID: str,
    bclFlowcellOutDir: str,
    flowcellBase: str,
    samplesBarcodesOrganismsDf: pd.DataFrame,
    config: dict[str:str],
) -> None:
    """
    ...
    def pushRunStats(
        ...
        def get_matrix_py_interop(bclFlowcellOutDir: str, flowcellBase: str) -> str:
            """Get run QC metrics using Python's interop library"""

            # Parse interop
            try:
                iop_df = pd.DataFrame(
                    interop.summary(interop.read(flowcellBase), "Lane")
                )
            except Exception:
                raise Exception(
                    "Cannot find/open Interop folder for flowcell {flowcellID}"
                )
            ...
            try:
                qdf = pd.read_csv(
                    os.path.join(bclFlowcellOutDir, "Reports", "Quality_Metrics.csv")
                )
            except Exception:
                raise Exception(
                    "Cannot find/open Quality_Metrics.csv for flowcell {flowcellID}"
                )
            ...
            for lane in list(qdf["Lane"].unique()):
                ...
                laneDict[laneStr]["cluster_pf"] = round(
                    subdf["YieldQ30"].sum() / subdf["Yield"].sum() * 100, 2
                )
        ...
        def get_matrix_illumina_interop(flowcellBase: str) -> str:
            ...
            try:
                ...
                interop_summary = subprocess.run(
                    interop_summary_cmd.split(), capture_output=True, text=True
                )
            ...
            headers = [
                c.strip() for c in [r for r in interop_summary if len(r) == 20][0]
            ]
            data = [
                [c.split(" ")[0] for c in r]
                for r in interop_summary
            ...
            try:
                ...
                raise Exception(
                    "Cannot find relevant headers in Interop summary for flowcell {flowcellID}"
                )
            ...
            data["Read"] = [
                i for i in range(1, num_reads + 1) for _ in range(num_lanes)
            ]
            ...
            new_headers = [
                ...
                "reads_pf",
                "read_{rn}_perc_q30",
                "read_{rn}_perc_aligned",
                "read_{rn}_error_rate",
                "read_{rn}_first_cycle_int",
                "read_number",
            ]
            ...
            data["read_{rn}_first_cycle_int"] = data[
                "read_{rn}_first_cycle_int"
            ].astype(int)
        ...
        pushParkourRunStat = requests.post(
            ...
            auth=(config.get("PARKOUR_USER"), config.get("PARKOUR_USER_PASSWORD")),
            data=d,
            verify=True,
            timeout=10,
        )
    ...
    def pushSequenceStats(
        ...
        try:
            demultiplexStatsdf = pd.read_csv(
                os.path.join(bclFlowcellOutDir, "Reports", "Demultiplex_Stats.csv")
            )
        except Exception:
            raise Exception(
        ...
        # Match demultiplexStatsdf and samplesBarcodesOrganismsDf on SampleID
        demultiplexStatsdf = pd.merge(
            demultiplexStatsdf, samplesBarcodesOrganismsDf, on="SampleID", how="left"
        )
        # Polish new df a bit
        demultiplexStatsdf = demultiplexStatsdf[
            ["Barcode", "SampleID", "# Reads", "Lane"]
        ]
        demultiplexStatsdf.columns = ["barcode", "name", "reads_pf_sequenced", "lane"]
        demultiplexStatsdf["barcode"] = demultiplexStatsdf["barcode"].fillna("")
        demultiplexStatsdf["reads_pf_sequenced"] = demultiplexStatsdf[
            "reads_pf_sequenced"
        ].astype(pd.Int64Dtype())
        demultiplexStatsdf["lane"] = demultiplexStatsdf["lane"].astype(pd.Int64Dtype())

        d = {"flowcell_id": flowcellID}
        d["sequences"] = demultiplexStatsdf[
            ["barcode", "name", "reads_pf_sequenced", "lane"]
        ...
        if pushParkourSequencesStats.status_code != 200:
            raise Exception(
                f"Cannot push run statistics for flowcell {flowcellID}. "
    ...
    try:
        pushSequenceStats(
            flowcellID, bclFlowcellOutDir, samplesBarcodesOrganismsDf, config
        )
...
def pullParkourSamplesBarcodesOrganisms(
    flowcellID: str, config: dict[str:str]
) -> pd.DataFrame:
    """
    ...
    if pullParkourFlowcellContents.status_code == 200:
        ...
        raise Exception(
            f"Cannot pull flowcell details for flowcell {flowcellID}. "
            f"Error: {pullParkourFlowcellContents.text}"
        )


def checkOrganismPrivacy(
    samplesBarcodesOrganismsDf: pd.DataFrame, config: dict[str:str]
) -> bool:
    # Check if any sample comes from an organism for which
    # the resulting data should be protected. If so, flag project
    # by creating a file called _TO_BE_PRIVACY_PROTECTED
    return any(
        o in config.get("PRIVACY_PROTECTED_ORGANISMS")
        for o in samplesBarcodesOrganismsDf["Organism"].unique()
    )
...
def main(bclFlowcellOutDir: str, flowcellBase: str, config: dict[str:str]) -> str:
    ...
    # Get Sample name <-> Barcode + Organism
    samplesBarcodesOrganismsDf = pullParkourSamplesBarcodesOrganisms(flowcellID, config)
    if checkOrganismPrivacy(samplesBarcodesOrganismsDf, config):
        open(os.path.join(flowcellBase, "_TO_BE_PRIVACY_PROTECTED"), "a").close()
    pushParkour(
        flowcellID, bclFlowcellOutDir, flowcellBase, samplesBarcodesOrganismsDf, config
    )
    return flowcellID
...
if __name__ == "__main__":
    try:
        ...
        flowcellID = main(cmd_args.bclFlowcellOutDir, cmd_args.flowcellDir, CONFIG)
    except Exception as exp:
